...
**Retrieval/plan boundary**: `CatalogResolver` validates the model's retrieval plan against an explicit allowlist of category slugs (defined inline in `conversation-dependencies.ts`) and a fixed set of endpoint shapes (search, category, generic browse, single-product detail, two-product comparison). `DUMMYJSON_BASE_URL` is a Zod literal pinned to `https://dummyjson.com` — it cannot be redirected via `.env`. Results are capped at six cards and ranked deterministically (exact match, then explicit sort, then relevance order, then product ID as tie-breaker) — see README's "Retrieval policy" table before changing ranking or plan validation.
