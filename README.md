...
The model path is bounded by the same discipline. `OPENAI_TIMEOUT_MS` (default 20,000 milliseconds) and `OPENAI_MAX_RETRIES` (default 1) are applied when the OpenAI client is constructed, replacing the SDK defaults of a 10-minute timeout and 2 retries, which would let a single hung planner call hold a request for roughly 30 minutes. `OPENAI_MAX_OUTPUT_TOKENS` (default 2,000) caps per-call cost on all three model calls — the plan is a small fixed object, the reply a short paragraph, the title 3-6 words. Because the planner is a reasoning model, that cap covers reasoning and visible output together, so it carries headroom above the visible text; a response truncated against the cap fails with an explicit truncation error rather than being reported as an empty result. Each model hop is therefore bounded at roughly 40 seconds worst case; a deadline shared across the whole planner → catalog → reply chain is not yet implemented.
...
A continuation request (a `search`/`browse_category` plan with `isContinuation: true`, e.g. "show me more" or "יש עוד") additionally excludes product IDs already shown earlier in the conversation from that same ranked candidate list before the six-card cap is applied. It is implemented by filtering the already-fetched local candidate pool — not by adding `skip`/`limit` pagination parameters to the DummyJSON request — and, for the live planner, by replaying the exact search/category/filters of the request that established the current context (tracked as `Message.retrievalAnchorMessage`) rather than parsing the continuation phrase itself. `isContinuation` is barred on `product_detail`, `compare`, `clarify`, and `unsupported` plans the same way an explicit `sort` is barred on those intents.

A rejected plan is not immediately terminal. The server re-prompts the planner exactly once, handing back the plan it rejected and the validator's reason, then re-validates; only a second rejection surfaces an error to the user. This is one bounded attempt, never a loop. Because the validator's reasons are specific ("Search plans require text and no product references"), most first-pass misses are self-correctable, and first-pass validity is tracked separately from post-repair validity so the two can be measured independently.

Ambiguous messages receive one targeted clarification when the missing detail would materially alter results; otherwise the assistant states the assumption it used. Off-catalog requests say that the current DummyJSON catalog cannot provide the item, rather than searching the web or inventing alternatives. One request produces one ranked product-card list of at most six items; separate result groups are not implemented, so multi-intent requests are clarified rather than merged into a mixed list — a "more"-style continuation of the same request is still a single ranked list per turn, not a merged multi-group response. User and catalog text are data, not instructions.
...
| Offline evaluation      | A deterministic regression test for the resolver, ranking, and grounding logic against versioned fixture scenarios (plan validity, intent, constraints, selected IDs, grounding, latency). | `npm run eval:offline` always runs `DeterministicModelClient`, a hand-written fake, never the real OpenAI planner, so it writes a JSON report under `artifacts/evaluations/` but cannot catch a planner-prompt regression; the fake never emits an invalid plan, so it never exercises the one-shot plan-repair path either. |
| Online smoke evaluation | The only suite that exercises the real planner: a small integration-health check against real OpenAI and DummyJSON.                                                                        | Optional, external, availability-dependent, and cost-capped to three scenarios; it is never CI-blocking.                                                                                                                                                                                                                     |

Deterministic tests assert structured effects and invariants, not exact assistant wording. Because the offline suite never calls the real model, a planner-prompt change should be validated with the online suite, not treated as covered once offline passes. The optional online command is intentionally opt-in and only runs when explicitly requested:
...
- Continuation ("show me more") exclusion only knows about product IDs shown in the latest twelve completed messages that reach the planner, and is bounded by the single up-to-100-item DummyJSON fetch per search/category/browse request; exhausting that pool ends continuation regardless of the upstream catalog's real size.
